...
void test(fpga_t* fpga) {
  ...
  for (dst_row = src_row + 1; dst_row < NUM_ROWS; dst_row++) {
    ...
    if (count > 250) {
      ...
      std::cout << dst_row - src_row << "," << dst_row << std::endl;
      subarray_size.insert(dst_row);
      src_row = dst_row;
  ...
  //========================================//
  // find distance between edge subarrays
  //========================================//
  src_row = 0;
  subarray_size.erase(subarray_size.begin());
  subarray_size.erase(subarray_size.begin());
  for (auto it = subarray_size.begin(); it != subarray_size.end(); it++) {
    iseq->size = 0;
    dst_row = *it-1;
    uint8_t src_pattern = 0xFF; 
    uint8_t dst_pattern = src_pattern;
    uint pattern_32 = src_pattern;
    for(int i = 0; i < 4-1; i++) {
      pattern_32 = (pattern_32 << 8) | src_pattern;
    }
    ...
    if (count > 250) {
      size.insert(*it);
      std::cerr << "Change src_row to " << *it << std::endl;
      src_row = *it;
      it++;
      it++;
    }
  ...
  std::cout << "Distance between edge subarrays = " << std::endl;
