...
void RowCopy(SoftMCPlatform *platform, uint bank, uint src_row, uint dst_row, uint wait) {
  ...
  // PRE
  p.add_inst(SMC_LI(dst_row, RAR));
  p.add_inst(SMC_NOP(), SMC_NOP(), SMC_NOP(), SMC_PRE(BAR, 0, 0));
  ...
  if (wait%4 == 0)
    ...
    p.add_inst(SMC_NOP(), SMC_ACT(BAR, 0, RAR, 0), SMC_NOP(), SMC_NOP());
  else if (wait%4 == 2)
    p.add_inst(SMC_NOP(), SMC_NOP(), SMC_ACT(BAR, 0, RAR, 0), SMC_NOP());
  else if (wait%4 == 3)
    p.add_inst(SMC_NOP(), SMC_NOP(), SMC_NOP(), SMC_ACT(BAR, 0, RAR, 0));
