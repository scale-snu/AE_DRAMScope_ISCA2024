...
# If xdma driver doesn't compiled, then compile it.
if [ $? -eq 0 ]; then
   rmmod xdma
fi

if [ ! -f ./linux-kernel/xdma/xdma.ko ]; then
    echo "Compling XDMA driver..."
    make clean -C ./linux-kernel/xdma
    make install -C ./linux-kernel/xdma
fi
...
insmod ./linux-kernel/xdma/xdma.ko
